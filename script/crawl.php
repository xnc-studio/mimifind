...
function get_mimi_articles(){
	...
	while (true) {
		...
		foreach ($json['list'] as $row) {
			...
			$province=(isset($row['province'])?base64_decode($row['province']):'');
			$city=(isset($row['city'])?base64_decode($row['city']):'');
			$rank=$row['rank'];
			$tags=implode(',', $row['tags']);
			$comment_num=(isset($row['public_comments_count'])?$row['public_comments_count']:0);
			$time_fetch=time();
			if($mid<=$latest_mid){
				$break_while=true;
				break;
